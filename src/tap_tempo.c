...
void TapTempo_Init(tap_tempo_t* tap_tempo, const tap_tempo_cfg_t* cfg)
{
	...
	tap_tempo->min_interval_ticks = tempo_period_ticks(cfg, MAX_TEMPO);
	tap_tempo->max_interval_ticks = tempo_period_ticks(cfg, MIN_TEMPO);
	tap_tempo->reset_interval_ticks = tempo_period_ticks(cfg, MIN_TEMPO) * INPUT_RESET_TO_MAX_TICKS_RATIO;
...
void TapTempo_ButtonPress(void* ctx)
{
	...
	if (num_intervals < MIN_INTERVALS_FOR_TEMPO)
		return;
...
static inline uint32_t tempo_period_ticks(const tap_tempo_cfg_t* tap_tempo_cfg, uint32_t tempo)
{
	return 60 * tap_tempo_cfg->tick_frequency / tempo;
}
